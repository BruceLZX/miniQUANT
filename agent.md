...
为避免大盘高流动性股票（如 AVGO）出现“>50 万美元大单过多、WF 量纲失真、Final Alpha 过大”的问题，D5 增加以下工程约束：

1. **大单阈值改为动态阈值（而非固定 50 万）**
   - `threshold_usd = max(500000, p95(1m_notional), 0.2% * day_notional)`
   - 解释：固定阈值在高成交额股票上过低，会把大量普通成交误判为大单。

2. **Whale Flow 全部改为美元口径归一化**
   - `BF = block_net_buy_value / ADV_dollar`
   - `DP = dark_pool_net / ADV_dollar`
   - `OF = options_whale_notional / ADV_dollar`
   - 其中 `ADV_dollar` 采用当日成交额（或 `price * averageDailyVolume3Month`）作为近似。

3. **WF 与 market alpha 增加稳健裁剪**
   - `BF/DP/OF` 单项裁剪到 `[-3, 3]`
   - `WF` 裁剪到 `[-4, 4]`
   - `market_alpha` 裁剪到 `[-6, 6]`
   - 目的：防止外部 API 异常值放大后直接主导仓位。

4. **仓位杠杆默认下调（更贴近稳健风格）**
   - 默认 `K: 1.0 -> 0.6`
   - 默认 `pos_max: 1.0 -> 0.8`
   - 含义：在波动率较低时避免动辄满仓，给 D6 风控与后验训练保留回旋空间。

5. **D5 使用数据范围说明**
   - 已使用：价格、成交量、VWAP 近似、买卖盘不平衡近似、大单净额/大单名义额（由 1m 全日序列估算）。
   - 暂缺逐笔暗池与逐笔期权鲸鱼成交时，用可解释近似项补位，但统一纳入门控与裁剪，不允许直接放大成极端仓位。
